...
import { getAnimation, useIsVisible, getWidgetState, getAppState, saveAppState } from "../../../container/js/utilities/utilities";
...
const VideoPlayer = forwardRef(({ parameters: propsParams, onEnded, onLoad, onPlay, onTimeUpdate }, ref) => {
  ...
  const { stopAudio, setSimulationStartTime, setUserName, setAvatarSelected, setStudentGrade, setTocState } = pageContext;
      ...
      player.on('play', () => {
        ...
        if(parameters.videoData.id === "introVideo" && !introVideoFirstPlayRef.current){
          ...
          if(savedState?.status === "NOT_STARTED"){
            saveAppState(pageContext, { status: "IN_PROGRESS" }, true);
          }
        }

        if(!previewMode && parameters.videoData.id)
          sendEvent('play', { 
  ...
  const skipVideoBtn = () => {
    ...
    const sliderSavedState = getWidgetState(pageContext, "slider");
    const userNameSavedState = getWidgetState(pageContext, "userNamePage");
    const avatarSavedState = getWidgetState(pageContext, "avatarPage");
    const scoreSavedState = getWidgetState(pageContext, "score");
    const tocSavedState = getWidgetState(pageContext, "mainScreen");
    if(userNameSavedState?.username) setUserName(userNameSavedState.username);
    if(avatarSavedState?.avatar) setAvatarSelected(avatarSavedState.avatar);
    if(scoreSavedState?.score) setStudentGrade(scoreSavedState.score);
    if(tocSavedState?.tocState) setTocState(tocSavedState.tocState);
    if (swiper) swiper.slideTo(sliderSavedState?.currentSlide || 1, 1);
  };
